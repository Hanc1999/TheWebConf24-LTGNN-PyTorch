...
def subgraph_batches_v3(A, unique_user_n, users, pos_items, neg_items, batch_size, K=3, batches=None):
    if isinstance(A, SparseTensor):
        ...
        # transfer the sparse matrix format to pyg.Data format
    ...
    for i in range(0, len(users), batch_size): # take batch on the user, take our their pos and neg items (one for each)
        ...
        unique_users_batch, user_inv_idx = torch.unique(u_batch, return_inverse=True) # index of sampled users among the root nodes
        ...
        real_batch_size = min(batch_size, len(users) - i) # deal with the corner case
